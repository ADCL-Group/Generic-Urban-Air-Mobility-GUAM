...
%     PropLim  =  [-0.001;  350.0]; % Propeller limits (rad/s)
    RotorLim  = [0;  SimIn.Model.Prop{1}.max_rpm]* 2*pi/60; % Rotor limits (rad/s)
    PropLim  =  [0;  SimIn.Model.Prop{nENG}.max_rpm]* 2*pi/60; % Propeller limits (rad/s)
...
Out.PosLim_hi = [RotorLim(2)*ones(nENG-1,1);PropLim(2)]; % (rad/s)
Out.PosLim_lo = [RotorLim(1)*ones(nENG-1,1);PropLim(1)]; % (rad/s)
