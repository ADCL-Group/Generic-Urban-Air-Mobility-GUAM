...
% Select vehicle model based on SimIn.vehID
if any(SimIn.vehID == [0 -1])
    vehModel = 'GUAM';
elseif SimIn.vehID == 1
    vehModel = 'HERO';
% elseif SimIn.vehID == 2
%     vehModel = 'SKYWALKER';
else
    error('Unknown vehID: %d', SimIn.vehID);
end

if strcmp(vehModel,'GUAM')
    % Lift + Cruise NASA model
...
elseif strcmp(vehModel,'HERO')
    % Makeflyeasy HERO VTOL
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Assumptions:
    %   - HERO parameters are extracted from build file.
    % Modifications:
    %   - 02/11/2026, MLF: Updated using 02/06/2026 version of HERO build.
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    % Mass properties
    % total mass [slug] - mass
    Model.mass= 0.416072625600000;
    % vehicle CG [ft] - cm_b
    Model.cm_b = [-0.002107579044020, 0.0, -0.001472132919542];
    % total moment of inertia tensor [slug-ft^2] - I
    Model.I = diag([0.333656866679198, ...
                    0.365679603021041, ...
                    0.691271848590261]);
    
    % Geometry properties
    % reference area [ft^2] - b_e * (c_root+c_tip)/2
    Model.S = 5.6703515161;
    % mean aerodynamic chord [ft] - from OVSP model
    Model.cbar = 0.8028215223097113;
    % wingspan [ft] - b
    Model.b = 7.145669519999999;
    % prop locations [ft] - Prop.p_b
    Model.Prop_location = ...
          [ 1.1318898,  1.1318898, -1.1318898, -1.1318898, -2.5115486368;
            -1.148294,  1.1482940, -1.1482940, 1.14829400, 0.0;
            -0.191043,  -0.191043, -0.1910433, -0.1910433, -0.0787401600];
    % prop diameter [ft] - Prop.Dp
    Model.Prop_D=[1.25000 * ones(1,4), 1.25000];
    % prop rotation direction (CW=+1, CCW=-1) - Prop.spin
    Model.prop_spin = [+1 -1 -1 +1 +1];
    % rotors - as viewed from below looking up
    %     Poly model: N1 => +N, N2 => -N
    %                 N3 => -N, N4 => +N
    % pusher prop - as viewed from the rear
    %     Poly model: N5 => +L
    % prop rotational moment of inertia [slug-ft^2]
    Model.Ip=[0.0002944295*ones(1,4), 0.0002944295];
    % prop orientation angles (roll-pitch-yaw) [deg]
    Model.Prop_angles=[  0,   0,   0,   0,   0
                       -90, -90, -90, -90,   0
                         0,  0,  0,   0,   0];
    % unit vector of the axis of rotation and hub rotation matrix
    Model.Prop_rot_axis_e=zeros(3,SimIn.numEngines);
